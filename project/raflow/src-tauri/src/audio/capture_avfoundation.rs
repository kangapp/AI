//! Audio capture using native CoreAudio API (AudioUnit)
//!
//! This module provides macOS-specific audio capture using CoreAudio's
//! AudioUnit API directly, bypassing cpal which has issues with `open` launch.
...
/// CoreAudio audio capture handle
...
impl AudioCapture {
    ...
    pub fn new() -> Result<Self, CaptureError> {
        ...
        tracing::info!(
            "[COREAUDIO] Using device ID {}, {} Hz, {} channels",
            device_id,
            sample_rate,
            channels
        );
        ...
        if status != 0 {
            tracing::warn!("[COREAUDIO] EnableIO returned: {}", status);
        }
        ...
        if status != 0 {
            tracing::warn!("[COREAUDIO] Disable output returned: {}", status);
        }
        ...
        if status != 0 {
            tracing::error!("[COREAUDIO] SetRenderCallback failed: {}", status);
            return Err(CaptureError::FormatError(format!(
                "Failed to set render callback: {}",
                status
            )));
        }
        tracing::info!("[COREAUDIO] Render callback set successfully on input scope");
    ...
    pub fn start(&mut self) -> Result<(), CaptureError> {
        ...
        tracing::info!("[COREAUDIO] AudioOutputUnitStart status: {}", status);
        if status != 0 {
...
fn get_default_input_device() -> Result<AudioDeviceID, CaptureError> {
    ...
    if status != 0 {
        tracing::error!("[COREAUDIO] Failed to get default input device: status={}", status);
        return Err(CaptureError::NoInputDevice);
    }

    if device_id == 0 {
        tracing::error!("[COREAUDIO] No input device available (device_id=0)");
        return Err(CaptureError::NoInputDevice);
    }

    tracing::info!("[COREAUDIO] Default input device ID: {}", device_id);
    Ok(device_id)
}
...
fn get_device_format(device_id: AudioDeviceID) -> Result<(u32, u32), CaptureError> {
    // Validate device ID
    if device_id == 0 {
        tracing::warn!("[COREAUDIO] Invalid device ID, using defaults");
        return Ok((44100, 1));
    }
    ...
    if status != 0 {
        tracing::warn!("[COREAUDIO] Failed to get sample rate, using default");
        sample_rate = 44100.0;
    }
    ...
    if status == 0 && size > 0 && size < 65536 {
        ...
        // Fallback: try kAudioHardwarePropertyDefaultInputDevice
        tracing::warn!("[COREAUDIO] Failed to get stream config size, trying alternative");
    }

    tracing::info!(
        "[COREAUDIO] Device format: {} Hz, {} channels",
        sample_rate as u32,
        channel_count
    );
...
unsafe extern "C" fn audio_input_callback(
    ...
    if count < 5 {
        tracing::info!("[COREAUDIO-CB] callback #{}, frames={}", count, in_num_frames);
    }
    ...
    if io_data.is_null() {
        tracing::warn!("[COREAUDIO-CB] io_data is null");
        return -1;
    }
    ...
    if total_samples > 0 {
        tracing::debug!("[COREAUDIO-CB] received {} samples, buffer len={}", total_samples, buffer.len());
    } else {
        tracing::warn!("[COREAUDIO-CB] no samples in io_data!");
    }
